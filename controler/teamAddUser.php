...
print_r($_POST);
    ...
    if(empty($un) || empty($pw) || empty($pwm) || empty($em)){
        ...
        $passedRegex = FALSE;
        header("Location: ../view/vendor/teamAddUser.php?message=char1");
        exit();
    }
    
    if($pw != $pwm){
        error_log("failed regex:".$_SESSION['user']."-".get_client_ip_env(), 0);
      header("Location: ../view/vendor/teamAddUser.php?message=char2");
      exit();
    }
    ...
    if (preg_match ('%^[A-Za-z0-9\.\' \-!_]{1,20}$%',$subjectUsername)) {
        ...
        header("Location: ../view/vendor/teamAddUser.php?message=char3");
        exit();
    }
    ...
    if (preg_match ('%^[A-za-z0-9\.\' \-!_&@$~]{1,20}$%', $subjectPassword)) {
        ...
        error_log("failed regex:".$_SESSION['user']."-".get_client_ip_env(), 0);
        $passedRegex = FALSE;
        header("Location: ../view/vendor/teamAddUser.php?message=char4");
        exit();
    }
    ...
    if (preg_match ('%^[A-za-z0-9\.\' \-!_&@$~]{1,20}$%', $subjectPasswordm)) {
        ...
        error_log("failed regex:".$_SESSION['user']."-".get_client_ip_env(), 0);
        $passedRegex = FALSE;
        header("Location: ../view/vendor/teamAddUser.php?message=char5");
        exit();
    }
    ...
    if (preg_match ('%^[A-za-z0-9\.\' \-!_&@.$~]{1,30}$%', $subjectEmail)) {
        ...
        error_log("failed regex:".$_SESSION['user']."-".get_client_ip_env(), 0);
        $passedRegex = FALSE;
        header("Location: ../view/vendor/teamAddUser.php?message=char6");
        exit();
    }
    ...
    if (preg_match ('%^[A-za-z0-9\.\' \-!_&@.$~]{1,30}$%', $subjectqty)) {
        ...
        error_log("failed regex:".$_SESSION['user']."-".get_client_ip_env(), 0);
        $passedRegex = FALSE;
        header("Location: ../view/vendor/teamAddUser.php?message=char7");
        exit();
    }
   ...
   if($passedRegex){
        ...
        if($numRows > 1){
            ...
            header("Location: ../view/vendor/teamAddUser.php?message=error");
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                if($username == $dbUsername){
                    ...
                    $UserNameFree = false;
                    header("Location: ../view/vendor/teamAddUser.php?message=username");
                    exit();
                }
            }
        } 
