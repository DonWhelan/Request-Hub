<?php
    ...
    function insert_prepared_companyUpload($dir, $name, $address, $address2, $postcode, $country, $expectedResult) {
        ...
        if (!$connection) {
            ...
            error_log("unable to connect! insert_prepared_companyUpload(),", 0);
            exit();
        }
        ...
        if($affectedRows != $expectedResult){
            error_log("unexpected result! insert_prepared_companyUpload(),", 0);
            return false;
        }else{
            return true;
        }
    ...
    function insert_prepared_companyUploadTransaction($dir, $name, $address, $address2, $postcode, $country, $expectedResult) {
        ...
        if (!$connection) {
            trigger_error("Could not reach database!<br/>");
            error_log("unable to connect! insert_prepared_companyUploadTransaction(),", 0);
            exit();
        }
        $db_selected = mysqli_select_db($connection, iDB);
        if (!$db_selected) {
        ...
        // check expected result
        if($affectedRows == $expectedResult){
            error_log("unaexpected result! insert_prepared_companyUploadTransaction(),", 0);
