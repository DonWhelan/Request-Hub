<?php
    ...
    */
    ...
    function insert_prepared_imageUpload($dir, $ufilename, $uhash, $uowner, $uvirusFree, $expectedResult, $randomID) {
        $connection = insertConnectionString($dir);
        // Check connection
        if ($connection->connect_error) {
            die("Connection failed: " . $connection->connect_error);
        }
        $stmt = $connection->prepare("INSERT INTO images (filename, hash, owner, virusFree, uploadID) VALUES (?,?,?,?,?)");
        $filename = $ufilename;
        $hash = $uhash;
        $owner = $uowner;
        $virusFree = $uvirusFree;
        $uploadID = $randomID;
        $stmt->bind_param("sssss", $filename, $hash, $owner, $virusFree, $uploadID);
        $stmt->execute();
    ...
    function insert_prepared_imageUploadTransaction($dir, $ufilename, $uhash, $uowner, $uvirusFree, $expectedResult, $randomID) {
        $connection = insertConnectionString($dir);
        mysqli_autocommit($connection,FALSE);
        mysqli_query($connection,"start transaction"); 
